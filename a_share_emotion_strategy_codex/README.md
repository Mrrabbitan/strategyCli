...
这是供当前电脑使用的只读研究页面。它把热点、四套策略和时点报告放在一起，记录依据、等待条件和风险，不管理账户，也不自动下单。
...
| 我的策略 | 分别阅读启动前潜伏 V3.4、一触即发、龙空龙、尾盘隔夜 | 四套规则独立，不互借门槛、资金参数或退出条件 |
...
- 专题可附带个股比较：必须说明比较池、方法、个股价格时点、反证、确认条件与来源。相同股票不得重复计数，同组原始排名只接受前三；展示研究顺序不授予交易资格。`related_modules` 可让同一专题出现在原四个研究入口的相关链接中，兼容原有 `related_module`；尾盘研究使用独立入口，页面共七个主入口。
