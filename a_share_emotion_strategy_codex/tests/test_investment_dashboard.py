...
class InvestmentDashboardTests(unittest.TestCase):
    ...
    def test_eight_pages_and_independent_rules_without_accounts(self):
        page=render_dashboard(dt.date(2026,9,11))
        for name in ('hot','dragon','yichujifa','prelaunch','late-day','three-step','strategy','reports'):self.assertIn(f'id="page-{name}"',page)
